...
void backPropagation(struct Neurones N, double inputs[N.inputs][1], 
        ...
        double gradient_output[N.output][1], double d_weight_oh[N.output][N.hidden], 
        double d_bias_o[N.inputs][N.inputs], double gradient_hidden[N.hidden][1], 
        double d_weight_ih[N.hidden][N.inputs], double d_bias_h[N.inputs][N.inputs], 
        ...
        double transpose_input[1][N.inputs], float lr)
{
	...
	transpose_matrix(N.output, N.hidden, weights_oh, transpose_hidden);
    ...
    //print_matrix(2,2,weights_ih);
    //print_matrix(2,2,weights_oh);
}
